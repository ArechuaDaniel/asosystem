...
import { startLoadingAso, startLoadingCantones, startLoadingPaises, startLoadingParroquias, startLoadingProvincias, updateAso } from "../store/asociacion/thunks";
...
const EditarAsociacion = () => {
            ...
            const [asociacions, setasociacions] = useState('')
            ...
            const {asociacion, paises, provincias, cantones, parroquias} = useSelector(state => state.asociacion)
            ...
            const idAsociacion = asociacion.idAsociacion
            ...
            useEffect(() => {  
                ...
                setasociacions(asociacion.asociacion)
                setDirector(asociacion.director)
                setFechaAfiliacion(formatearFecha(asociacion.fechaAfiliacion))
                setTelefono(asociacion.telefono)
                setCorreo(asociacion.correo)
                setIdParroquia(asociacion.idParroquia)
                setDireccion(asociacion.direccion)
                setIdPais(asociacion.idPais)
                setIdProvincia(asociacion.idProvincia)
                setIdCanton(asociacion.idCanton)
            }, [asociacion])
            useEffect(() => {
            ...
            const handleSubmit = async e => {
                ...
                if ([asociacions, director, fechaAfiliacion, telefono, correo, idParroquia, direccion].includes('')) {
                    Swal.fire({
                ...
                try {
                   ...
                   dispatch(updateAso({asociacions, director, fechaAfiliacion, telefono, correo, idParroquia, direccion, idAsociacion}))
                   navigate('/asosystem/api/perfil') 
                } catch (error) {
                    console.log(error);
                    setAlerta({
                            ...
                            <div className='rounded-xl bg-white '>
                                <h1 className='text-sky-600 font-black md:text-3xl text-2xl px-10 mt-10'>
                                    <span className="material-symbols-outlined align-middle text-3xl mr-2">
                                        person
                                    </span>Actualizar datos de la Asociación  </h1>
                                ...
                                <form
                                    ...
                                    <div className='my-5'>
                                        <label className='capitalize text-gray-600  text-xl font-bold' htmlFor='asociacion'>Asociación*</label>
                                        <input
                                            type='text'
                                            id='asociacion'
                                            placeholder='Ingresa nombre de la Asociación'
                                            className='capitalize w-full mt-3 p-3 border rounded-xl bg-gray-50'
                                            value={asociacions}
                                            onChange={e => setasociacions(e.target.value)}
                                        />
                                    </div>
                                    <div className='my-5'>
